...
@Component
@Slf4j
public class AcLogoutSuccessHandler implements LogoutSuccessHandler {
    @Autowired
     ...
     * @param user
     */
    private  void expireSession(HttpServletRequest request, AcUserDetails user) {
        List<SessionInformation> sessionsInfo = null;
        if (null != user) {
            List<Object> o = sessionRegistry.getAllPrincipals();
            for (Object principal : o) {
                if ((user.getUsername().equals(((AcUserDetails) principal).getUsername()))) {
